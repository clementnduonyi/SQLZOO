...
--Run the query and notice the two services that link these stops have a count of 2. Add a HAVI
...
--stops by name rather than by number. Change the query so 
...
SELECT DISTINCT a.company, a.num FROM route a
    JOIN route b ON a.num = b.num 
        AND a.company = b.company
            JOIN stops stopsa ON
                stopsa.id = a.stop
                    JOIN stops stopsb ON
                        stopsb.id = b.stop
...
--Hint
--Self-join twice to find buses that visit Craiglockhart and 
--Lochend, then join those on matching stops.
